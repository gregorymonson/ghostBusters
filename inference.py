...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = [self.legalPositions[k % len(self.legalPositions)] for k in range(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for particle in beliefDist:
                ...
                beliefDist[particle] = beliefDist[particle]*emissionModel[trueDistance]

            if beliefDist.totalCount() == 0.0:
                self.initializeUniformly(gameState)
            else:
                beliefDist.normalize()
                self.particles = [util.sample(beliefDist) for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for ghostIndex in range(self.numGhosts):
            if noisyDistances[ghostIndex] == None:
                updatedParticles = []
                for particle in self.particles:
                    updatedParticle = self.getParticleWithGhostInJail(particle, ghostIndex)
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] != None:
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[ghostIndex])
                    weights[particle] *= emissionModels[ghostIndex][trueDistance]
        ...
        if weights.totalCount() == 0.0:
            ...
            beliefDist = self.getBeliefDistribution()
            for particle in self.particles:
                beliefDist[particle] *= weights[particle]
                weights[particle] = 1.0
            beliefDist.normalize()
            self.particles = [util.sample(beliefDist) for i in range(self.numParticles)]
